...
- 复杂页面内容进入 feature page/container、view-model hook 和 components。
...
3. 检查 public / protected route 是否落在正确 shell。
4. 检查 debug route 是否通过 runtime/development split 隔离生产。
5. 检查 `routeTree.gen.ts` 是否只来自生成流程。
