...
Binance reads like a financial trading platform that wants to feel both authoritative and energetic. The base atmosphere is **clean white canvas** (`{colors.canvas-dark}` — #ffffff) holding dark type and a single, ubiquitous accent: **QuantAgent Blue** (`{colors.primary}` — #3b82f6). That blue does almost all of the brand's heavy lifting — it carries every primary CTA, every value-claim headline ("FUNDS ARE SAFU"), every "Sign Up" pill, every featured tier indicator, and the wordmark itself. There is no secondary brand color. The system trusts the blue voltage to do the brand work, and it carries it.
...
The product is **light-theme**: all surfaces (homepage, smart-money, futures arena, transactional) use a clean white canvas. The same blue CTAs and gray hairlines (`{colors.hairline-on-light}` — #e5e7eb) thread through all surfaces. Trading **green** (`{colors.trading-up}` — #0ecb81) and **red** (`{colors.trading-down}` — #f6465d) signal price direction in tables, charts, and price tickers.

**Key Characteristics:**
- Single accent color: `{colors.primary}` (#3b82f6) does all brand voltage — primary CTAs, hero headlines, brand mark, badges. Used as the primary accent across all surfaces.
...
- Light theme: all pages use white canvas (`{colors.canvas-dark}` → #ffffff) with light gray card surfaces. Blue CTAs and trading green/red are consistent throughout.
- Footer surface: the homepage uses `{colors.surface-soft-light}` (#fafafa) for the footer — a subtle surface step that visually closes the page.
...
- Card surfaces: `{colors.surface-card-dark}` (#f5f5f5) for elevated cards; `{colors.canvas-light}` (#ffffff) for base surfaces. No gradient surfaces, no atmospheric backdrops — flat color blocks throughout.
...
### Brand & Accent
- **QuantAgent Blue** (`{colors.primary}` — #3b82f6): The single brand color. Used for primary CTA backgrounds, the wordmark, brand-claim headlines ("FUNDS ARE SAFU"), trust badges ("No.1 Trading Volume"), large stat numbers in `{component.stat-callout-card}`, and inline links.
- **QuantAgent Blue Active** (`{colors.primary-active}` — #2563eb): The press / hover-darker variant. Deeper blue.
- **QuantAgent Blue Disabled** (`{colors.primary-disabled}` — #93c5fd): A desaturated light-blue used on disabled CTAs over light canvas.
- **Accent Turquoise** (`{colors.accent-turquoise}` — #2dbdb6): A small secondary accent used very sparingly on Smart Money's "Check Now" CTA. Treat as a single-product accent, not a system color.

### Surface

The system uses a light canvas as the default surface:

**Light mode (default):**
- **Canvas Dark** (`{colors.canvas-dark}` — #ffffff): The primary page floor. Clean white — the base for all surfaces.
- **Surface Card Dark** (`{colors.surface-card-dark}` — #f5f5f5): Cards, navigation dropdowns, secondary buttons, markets table.
- **Surface Elevated Dark** (`{colors.surface-elevated-dark}` — #e5e7eb): One step darker, used for nested cards, hovered nav items, and chart background panels.

**Light surfaces:**
- **Canvas Light** (`{colors.canvas-light}` — #ffffff): The page floor on all pages.
...
### Hairlines & Borders
- **Hairline on Light** (`{colors.hairline-on-light}` — #e5e7eb): The 1px border tone on light surfaces. Hairlines are used liberally for table dividers, card borders, and section separators.
- **Hairline on Dark** (`{colors.hairline-on-dark}` — #d1d5db): The 1px border tone for slightly stronger separation on light surfaces.
- **Border Strong** (`{colors.border-strong}` — #cdd1d6): A heavier border tone used on disabled secondary buttons.

### Text
- **Ink** (`{colors.ink}` — #181a20): The strongest text on light surfaces. Display headlines.
- **Body** (`{colors.body}` — #374151): Default running-text on white canvas — dark gray for comfortable reading.
- **Body on Light** (`{colors.body-on-light}` — #181a20): Same as ink — body text reuses the ink token for emphasis.
- **Muted** (`{colors.muted}` — #6b7280): Footer links, breadcrumbs, captions, table column headers. Secondary text color.
- **Muted Strong** (`{colors.muted-strong}` — #4b5563): A second-tier muted for emphasized labels and body text.
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on blue primary CTAs.
- **On Dark** (`{colors.on-dark}` — #181a20): Dark text for high-contrast headlines on light canvas.
...
The elevation philosophy is **flat surfaces with color-block separation**. Binance does not use heavy drop shadows or glassmorphism — depth comes from the contrast between `{colors.canvas-dark}` (#ffffff) and `{colors.surface-card-dark}` (#f5f5f5) (a subtle lightness step that reads as a clear elevation boundary).

### Decorative Depth
- **Blue → white vertical gradient backdrop** on the Futures Arena hero: `{colors.primary}` fading down to `{colors.canvas-dark}`. This is a single-page treatment used for product-launch / event hero surfaces, not a system-wide signature.
...
**`top-nav-dark`** — The top nav on white canvas. 64px tall, `{colors.canvas-dark}` background. Carries the blue wordmark at left, primary horizontal menu (Buy Crypto, Markets, Trade, Futures, Earn, Square, Smart Money, Campaigns), right-side cluster with language selector, light/dark toggle, "Log In" text link, "Sign Up" `{component.button-primary}`. The wordmark uses `{colors.primary}` for brand type.

**`top-nav-light`** — Same layout as top-nav, `{colors.canvas-light}` background and `{colors.ink}` menu items.

### Buttons

**`button-primary`** — The signature primary CTA. Background `{colors.primary}`, text `{colors.on-primary}` (white on blue — the system's iconic combination), type `{typography.button}`, padding 12px × 24px, height 40px, rounded `{rounded.md}` (6px). Press state: `button-primary-active` darkens to `{colors.primary-active}` (#2563eb). Disabled state: `button-primary-disabled` desaturates to `{colors.primary-disabled}`.

**`button-primary-pill`** — A larger pill variant of the primary CTA used for top-of-page sign-up moments and product-launch heroes (Futures Arena "Join Now"). Same blue + white combination, padding 14px × 32px, rounded `{rounded.pill}` (9999px). Use sparingly — the pill is a "this is THE action" signal.
...
**`button-subscribe`** — Compact blue CTA used in the Smart Money traders table to subscribe to a top trader. Smaller height (28px) and tighter padding than the primary CTA — fits inside dense table rows. Same blue + white combination.

**`text-link`** — Inline body links in `{colors.primary}` (blue on white). No underline by default. Type inherits `{typography.body-md}`.

### Cards & Containers

**`hero-band-dark`** — Full-width white band carrying the homepage h1 + sub-headline + dual CTA pair. Background `{colors.canvas-dark}` (#ffffff), padding `{spacing.section}` (80px). The h1 ("316,258,026 USERS TRUST US") uses `{typography.hero-display}` at 64px / 700 — the system's largest type role.

**`stat-callout-card`** — Inline blue stat numbers (15,000 BTC, 7,488,223, $429,423,449). Transparent background, text `{colors.primary}`, type `{typography.number-display}` in BinancePlex. Used as a flat layout block, not a card with surface — the blue text alone carries the visual weight.

**`trust-badge`** — Small light cards holding "No.1 Customer Service" / "No.1 Trading Volume" claims. Background `{colors.surface-card-dark}` (#f5f5f5), rounded `{rounded.lg}` (8px), padding 16px × 20px. Blue numeric or word badge ("No.1") sits next to a short label.
...
**`funds-safu-band`** — The blue-headlined "FUNDS ARE SAFU" band. Background stays `{colors.canvas-dark}` (#ffffff), but the headline uses `{colors.primary}` at `{typography.display-lg}`. Below the headline, three large `{component.stat-callout-card}` numbers anchor the band: total BTC reserves, users helped, funds recovered.
...
**`cta-band-dark`** — The "Secure, Low-Fee Trading on Binance" pre-footer CTA band. Background `{colors.surface-card-dark}` (#f5f5f5, one step elevated from canvas), rounded `{rounded.xl}`, padding `{spacing.xxl}` (48px). Carries an h2 in `{typography.display-sm}` and a `{component.button-primary}` aligned right.

### Light-Mode Transactional Components

**`buy-crypto-amount-card`** — The right-rail card on the Buy BTC page. Background `{colors.canvas-light}`, rounded `{rounded.lg}` (8px), padding `{spacing.lg}` (24px). Carries an editable amount input in `{typography.number-display}` (BinancePlex), a currency selector, and a blue `{component.button-primary}` for "Continue" / "Confirm Order."
...
**`search-input-on-dark`** — The "Search currencies" input on the homepage hero. Background `{colors.surface-card-dark}`, text `{colors.on-dark}`, rounded `{rounded.lg}` (8px), padding 10px × 16px, height 40px. Carries a blue `{component.button-primary-pill}` on the right side ("Sign Up").
...
**`trader-row`** — A single row in the top-traders table on /smart-money. Transparent background, padding 12px vertical, hairline divider between rows. Avatar + trader name + private/public badge on left; ROI %, AUM, mint date columns; blue `{component.button-subscribe}` on right.
...
**`footer-light`** — The light-gray footer that closes every page. Background `{colors.surface-soft-light}` (#fafafa), text `{colors.body-on-light}`. 6-column link list at desktop covering Community / About Us / Products / Business / Service / Learn columns. Vertical padding 64px. The subtle surface step from white canvas to gray footer visually closes the page with a "marketing reset" surface.

## Do's and Don'ts

### Do
- Reserve `{colors.primary}` (QuantAgent Blue) for primary actions, brand-claim headlines, and the wordmark. Never use it for secondary or decorative purposes — blue's scarcity is what makes it powerful.
- Keep `{component.button-primary}` (blue with white text) as the universal primary CTA across all surfaces. The same button appears identically on `{colors.canvas-dark}` and `{colors.canvas-light}`.
...
- Don't use blue for body text or large surface fills. It is for focal-point CTAs and headlines only.
...
- Don't invert `{component.button-primary}`'s text color. White on blue is the system's signature — dark text on blue loses contrast and brand recognition.
...
### Collapsing Strategy
- Top nav collapses to hamburger at < 768px; the menu opens as a full-screen sheet with the same blue accent CTAs anchored to the bottom of the sheet.
