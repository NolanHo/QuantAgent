...
- Web 前端、插件后台、Agent 运行调试、HITL 授权、通知和 broker 能力都必须有可查询、可审计接口；初版 broker 只做虚盘，不操作实盘。
...
- 插件安装、启停、reload 状态变化。
- Scheduler run 状态变化。
- Notification / broker 状态变化；初版 broker 状态只覆盖虚盘，不操作实盘。
- Runtime error 提醒。
...
| `link_confirm` | 需要一次性链接确认 | 高风险交易请求、启用 broker |
...
- 插件配置、API key、HITL 授权、broker 能力都是敏感能力，不能完全裸奔；初版 broker 只做虚盘，不操作实盘。
...
/brokers
/brokers/{executor_id}/actions/dry_run
