...
- Executor：dry_run / mock 能力和 disabled 状态。
...
- 高风险动作二次确认。
- executor 类型必须明确初版只支持 disabled / dry_run / mock。
- 启用前必须显示依赖和配置检查结果。
- 所有动作写审计。
...
| executor | disabled / dry_run / mock、Policy Gate、权限和最近 dry_run |
...
| executor disabled | Ops tab 解释不能执行任何动作 |
...
- 用户能看到 executor 初版只支持 disabled / dry_run / mock。
