...
- 关键意图必须写中文注释：安全边界、审计、状态机、权限、并发、幂等、重试、协议适配、生成物边界和非显然取舍必须用短中文注释说明“为什么这样做”；不要默认只写英文注释或完全不注释。
- 公共模块、共享组件、复杂 feature 能力和非显然目录都必须提供 README 或最小 usage note，说明职责、边界、入口和不要放什么；不能把这些约束只藏在代码命名里。
- 目录结构也必须遵守职责分层。不要为了“都在同一模块里”就在一个目录下平铺几十个文件；当同类文件增长到会影响定位时，必须按 `components/`、`hooks/`、`api/`、`types/`、`utils/`、`README.md` 等职责分组，而不是无序堆放。
