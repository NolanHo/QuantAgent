...
- 将现有评分 mock contract 映射为详情页可消费的稳定模型。
- `components/analysis/` 只放行业影响、最佳动作和证据质量这类展示面板，不放 mock 查找、route 参数或请求逻辑。

当前阶段不负责：

- 真实 API 请求、TanStack Query 或 generated client 接入。
- `/events` 事件中心列表与筛选页的实现。
- 审批 mutation、真实执行、权限放行或 Runtime 详情页逻辑。

不要继续放入：

- 不要把 route 参数读取和页面主体 JSX 重新堆回 route 文件。
- 不要让 view 组件直接依赖 `event-scoring.mock` 的原始字段结构。
