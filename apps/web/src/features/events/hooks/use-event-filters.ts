...
export function useEventFilters() {
  ...
  const params = useMemo(
    () => toEventQueryParams(filters, timeWindowAnchorMs),
    [filters, timeWindowAnchorMs],
  );
  ...
  return {
    ...
    setTime(time: EventFilterState['time']) {
      // 中文注释：滚动时间窗只在用户切换时间筛选时重新取锚点，避免 render 时 Date.now() 改变 query key 造成重复请求。
      setTimeWindowAnchorMs(Date.now());
      setFilters((current) => ({ ...current, time }));
    },
  };
}

export function toEventQueryParams(filters: EventFilterState, timeWindowAnchorMs: number): EventQueryParams {
  return {
    keyword: filters.keyword || undefined,
    decision: decisionFromFilters(filters),
    industry_id: industryIdFromFilter(filters.industry),
    sort: filters.sort,
    time_from: timeFromFilter(filters.time, timeWindowAnchorMs),
    limit: 50,
  };
}
...
function timeFromFilter(time: EventFilterState['time'], anchorMs: number): string | undefined {
