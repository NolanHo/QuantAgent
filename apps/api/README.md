...
Discord approval loop 当前只要求发送 webhook 通知，不要求 Discord 回流收消息。真实测试路径是：
...
Agent submit_action_plan
  ...
  -> worker 读取插件配置中的 webhook_url
  -> Discord webhook send
  -> 用户到 Web /approvals 审批
```

worker 默认使用 Discord notification 插件：
...
Discord webhook URL 不通过环境变量配置，也不存在 `NOTIFICATION_DISPATCH_PLUGIN_CONFIG`。必须在 Web 插件配置管理中为 `quantagent.official.notification.discord` 保存 `webhook_url`；该字段按 sensitive 配置加密入库，API 响应只返回掩码，worker 运行时解密后只以内存配置传给插件。

`POST /api/v1/integrations/notifications/ingress` 是通用 notification ingress 的长期预留入口。它直接返回目标 notification 插件给出的原生 response，不走项目统一 `code/data/msg/error` envelope。当前 NVDA 财报真实测试不使用 Discord receive，不需要 public key、application id、guild/channel allowlist，也不通过 Discord 文本 approve 授权。
