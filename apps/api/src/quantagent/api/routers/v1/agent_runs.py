...
from quantagent.api.schemas.runtime_inspect import AgentRunDetail, AgentRunSummary, RuntimeListResponse
...
router = APIRouter(prefix="/agents/runs", tags=["runtime"])


@router.get("", response_model=ApiResponse[RuntimeListResponse[AgentRunSummary]])
def list_agent_runs(
    request: Request,
...
) -> ApiResponse[RuntimeListResponse[AgentRunSummary]]:
    ...
    return ApiResponse.success(service.list_agent_runs(filters=filters))
...
def get_agent_run(
    ...
    service = RuntimeInspectService(session=session, request=request)
    return ApiResponse.success(service.get_agent_run(run_id))
