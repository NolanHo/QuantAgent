...
from quantagent.api.schemas.runtime_inspect import RuntimeListResponse, ToolInvocationDetail, ToolInvocationSummary
...
router = APIRouter(prefix="/tools/invocations", tags=["runtime"])


@router.get("", response_model=ApiResponse[RuntimeListResponse[ToolInvocationSummary]])
def list_tool_invocations(
...
) -> ApiResponse[RuntimeListResponse[ToolInvocationSummary]]:
    ...
    return ApiResponse.success(service.list_tool_invocations(filters=filters))


@router.get("/{invocation_id}", response_model=ApiResponse[ToolInvocationDetail])
def get_tool_invocation(
    invocation_id: str,
    request: Request,
    session: Session | None = Depends(get_optional_db_session),
    _actor: CurrentActor = Depends(require_capability(RUNTIME_INSPECT_CAPABILITY)),
) -> ApiResponse[ToolInvocationDetail]:
    service = RuntimeInspectService(session=session, request=request)
    return ApiResponse.success(service.get_tool_invocation(invocation_id))
