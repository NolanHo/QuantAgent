...
- `approve` -> `ApprovalInput(id=<input_id or generated>, approval_id=<approval_id>, channel="web", actor_ref=<actor>, raw_text=<reason/comment summary>, structured_payload={"intent": "approve", ...})`
- `reject` -> `ApprovalInput(id=<input_id or generated>, approval_id=<approval_id>, channel="web", actor_ref=<actor>, raw_text=<reason/comment summary>, structured_payload={"intent": "reject", ...})`
- `request-reanalysis` -> `ApprovalInput(id=<input_id or generated>, approval_id=<approval_id>, channel="web", actor_ref=<actor>, raw_text=<reason/comment summary>, structured_payload={"intent": "request_reanalysis", ...})`
...
`request-reanalysis` action 转换为带完整必填字段的 `ApprovalInput(id=<input_id or generated>, approval_id=<approval_id>, channel="web", actor_ref=<actor>, structured_payload={"intent": "request_reanalysis"})`，经 evaluator 生成 `ApprovalEvaluation`，最终写入 `ApprovalDecisionStatus.REANALYSIS_REQUESTED` 和 audit record。V1 不发布 reanalysis event、不创建 Agent run、不调用 worker/scheduler，也不在 HTTP 请求内同步启动分析。
