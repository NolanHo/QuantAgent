...
这一刀先只收住官方 `Jina Reader` Source Plugin 的插件包能力、配置契约、外部 reader 使用边界和平台约定的 source 输出结构，让后续 crawler / search 类插件能在与 `Readability` 平行的边界上复用第二条 reader 路径。

## What Changes

- 定义官方 `Jina Reader` Source Plugin 的插件包边界和最小目录结构。
- 明确插件只能通过 `plugin.yaml` 和 Registry 进入系统，不要求核心代码为该插件增加硬编码注册逻辑。
...
- 定义插件输出为平台约定的 Source Plugin 输出结构 / source runtime 可消费输出 DTO，不提前绑定 core 内部 DTO 名称。
- 明确插件只负责通过 `Jina Reader` 类服务读取内容、标准化输出和清晰失败返回，不负责 `RawEvent` 入库、去重、`SourceBinding`、Event Bus、权限和生命周期。
- 明确插件遵循平台传入的允许/禁止外发策略结果、调用 DTO 或 `effective_config`；当平台未授权外发时，插件返回清晰拒绝或失败信息。

## Capabilities

### New Capabilities

- `source-jina-reader`: 官方 `Jina Reader` Source Plugin 通过 `plugin.yaml`/Registry 接入，对普通网页 URL 执行外部 reader 读取和文本标准化，返回平台约定的 Source Plugin 输出结构，供平台写入事件链路。
