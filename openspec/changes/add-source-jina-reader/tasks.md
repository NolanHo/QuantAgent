...
- [ ] 1.2 固定插件输出为平台约定的 Source Plugin 输出结构 / source runtime 可消费输出 DTO，不新增 reader 专用 DTO，也不提前绑定 core 内部 DTO 名称。
- [ ] 1.3 固定插件不负责 `RawEvent` 入库、去重、`SourceBinding`、Event Bus、权限和生命周期。
- [ ] 1.4 固定插件只能通过 `plugin.yaml` 和 Registry 接入，不要求核心代码为该插件增加硬编码注册逻辑。
...
- [ ] 2.3 明确外发允许/禁止由平台 policy 结果、调用 DTO 或 `effective_config` 传入；插件只执行已授权请求，在未授权时清晰拒绝或失败返回。
