...
### 8. issue #130 的事件详情先消费评分摘要，不等待真实 DTO 一次性到位

`issue #130` 需要把 `/events/:eventId` 从占位骨架收口为事件详情 / 决策页 V1，但本轮并不要求同时敲定真实详情 API DTO、`packages/contracts` 或 generated client。现有评分 change 只要求正式字段语义保持稳定，不要求事件详情首版必须等待真实 contract 才能实现。

因此本轮实现边界应固定为：

- 事件详情 feature 可以先在内部适配现有 mock scoring contract，作为页面编排的临时输入边界。
- route、业务 hook 和展示组件不得直接绑定 `event-scoring.mock` 原始 shape；需要先映射为事件详情页消费模型。
- 当真实 DTO / contracts ready 后，应只替换 adapter / page model 边界，而不改写页面职责、阅读顺序或评分分层语义。

这允许前端先把“事实、评分解释、最佳动作、审批入口、运行摘要”收成稳定页面能力，同时不把当前 mock 字段扩散成长期真源。

### 9. 事件详情的评分摘要必须与最佳动作和 gate 上下文成组出现

事件详情页不是单纯展示分数的看板，而是用评分帮助操盘者理解“为什么系统给出这个最佳动作、当前是否还需要人工确认、哪里存在不确定性”。因此 `issue #130` 的实现需要固定以下组合关系：

- `source_authority`、`event_reliability`、`verification_status` 紧贴事件事实与验证状态展示。
- `impact_strength`、`analysis_confidence`、`recommendation_score` 与最佳动作、风险方向、确认链路一起出现，而不是漂浮成孤立 badge。
- `uncertainty_summary`、冲突摘要、工具失败或 Policy Gate 阻断进入辅助解释区，帮助用户理解为什么高分不等于可执行。
- 中文注释需要明确“建议推荐度、分析置信度、事件可信度都不是执行放行信号”，避免后续实现把评分语义错误投射到 CTA 文案。

## Responsibility Blueprint

本 change 只固定评分语义和实现前 gate，不直接创建真实 API、contracts 或 Web feature。后续实现进入代码时必须按以下职责收口：

| 边界 | 负责什么 | 不负责什么 |
| --- | --- | --- |
| `event-scoring-v1` OpenSpec | 评分层语义、页面消费矩阵、降级场景、执行权限边界、mock 到正式 contract 的收口规则 | 评分算法、真实 DTO 字段类型、数据库结构、生成命令、页面组件实现 |
| API DTO / `packages/contracts` | 正式评分摘要字段、跨端字段命名、序列化 contract、生成或校验入口 | 页面筛选状态、展示 label、mock-only 字段、执行放行策略 |
| `/events` feature | 首次正式消费评分摘要，承接重点事件、排序、筛选、入选原因和列表降级展示 | Dashboard 总控职责、审批动作、完整评分解释页 |
| Dashboard feature | 后续只消费重点事件摘要，用于首页第一判断 | 首次定义评分字段、完整事件筛选、完整评分解释 |
| Event Detail feature | 将评分解释与事件事实、最佳动作、审批入口和 gate 上下文成组展示 | 直接批准、真实执行、完整模型推理链、长期依赖原始 mock shape |
| Approvals feature | 消费推荐度、可信度、置信度、风险方向、确认等级和到期策略作为人工确认上下文 | 用评分替代 Policy Gate、把批准表达成真实执行完成 |

正式 Web 实现的文件职责必须遵守 Web gate：

- route 只负责 `createFileRoute`、search / path params、loader / beforeLoad 和页面装配。
- feature API / contracts 只表达 endpoint、payload、params、response type 和 DTO 映射边界。
- query keys、query hooks、business hooks、components、types、utils 和 README 按职责拆分。
- 评分 label、等级映射、排序参数、筛选参数、降级原因和空 / 错状态不得散落在 route 或单个展示组件内。
- 当前 mock 适配只能落在 feature 内 adapter / page model 边界；展示组件不直接依赖原始 mock DTO shape。

## Contract Ownership Boundaries

正式评分字段进入 API 或 `packages/contracts` 前，必须先明确字段所有权和状态真源：

- `source_authority`、`event_reliability`、`impact_strength`、`freshness`、`event_priority`、`analysis_confidence`、`recommendation_score`、`verification_status`、`uncertainty_summary` 是评分摘要语义。
- `risk_direction`、`risk_level`、`confirmation_level` 是审批和风险治理语义，只能相邻展示，不能混入评分层。
- `priority`、`referenceStrength`、`industryImpact` 等首页骨架字段是 mock-only 表达，不能进入正式 API DTO、contracts、generated client 或长期前端类型。
- WebSocket 只能触发 query invalidation；评分状态恢复、刷新和审计回放以 REST、数据库和审计记录为真源。
