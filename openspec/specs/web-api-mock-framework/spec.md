# Web API Mock Framework Specification

## Purpose

定义 `apps/web` 测试体系中的共享 network mock envelope、Playwright route mock 与 401 recover 预留表达边界。
...
网络 mock 框架 SHALL 为 401 unauthorized 与 recover 测试预留表达能力，但本 capability MUST NOT 固化真实 refresh/cookie 实现。
...
- **AND** 本 capability 不固定具体业务 endpoint、cookie 行为或最终请求重放实现
- **AND** 本 capability 不实现真实 `apiClient` interceptor 或 refresh 流程
